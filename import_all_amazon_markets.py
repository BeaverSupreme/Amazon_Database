...
# -*- coding: utf-8 -*-
"""
run_database_ui.py — Serwer UI z ARCHIWUM CEN Z POPRZEDNIEGO ROKU (2025 r. vs 2026 r.)
======================================================================================
ZAKTUALIZOWANA WERSJA ARCHIWALNA:
1. Pokazuje porównanie cen rok do roku (np. "📜 2025: 469 zł | 📉 Taniej o -15%").
2. Wyszukiwarka ofert i Kafelki szybkiego wyszukiwania z obsługą danych historycznych.
3. Pełna kompatybilność z 4 rynkami (PL, UK, DE, FR) oraz 4 platformami.
...
def heal_and_migrate_database(silent=False):
    try:
        ...
        cursor.execute("SELECT asin, country_code, category_slug, sales_volume, price, price_1y_ago FROM products;")
        all_rows = cursor.fetchall()

        updates = []
        sales_tiers = [100, 200, 300, 500, 800, 1000, 1500, 2500, 4000, 6500, 10000, 15000, 25000, 40000]

        for asin, country_code, category_slug, sales_vol, price, p_1y in all_rows:
            needs_update = False
            good_asin = asin
            brand = "Generic"
            ...
            if needs_update:
                ...
                updates.append((good_asin, brand, title, new_url, new_vol, new_p_1y, asin, country_code))

        if updates:
            cursor.executemany("""
                UPDATE products
                SET asin = ?, brand = ?, title = ?, url = ?, sales_volume = ?, price_1y_ago = ?
                WHERE asin = ? AND country_code = ?;
            """, updates)
            conn.commit()
            if not silent:
                print(f"[AUTO-HEALING] Zaktualizowano {len(updates)} produktów o archiwum cen 2025 r. i linki!")
        conn.close()
        return True
...
def main():
    parser = argparse.ArgumentParser(
        description="Importer dla 4 kluczowych rynków Europy (PL, UK, DE, FR) z archiwum cen z 2025 roku"
    )
