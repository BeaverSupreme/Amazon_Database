...
# -*- coding: utf-8 -*-
"""
import_all_amazon_markets.py — Importer WSZYSTKICH PRODUKTÓW dla 21 RYNKÓW AMAZON (0 zł / Chmura)
================================================================================================
ZAKTUALIZOWANA WERSJA:
1. Pełny katalog WSZYSTKICH PRODUKTÓW (nie tylko bestsellery — wszystkie kategorie i rynki!).
2. Zaktualizowane nazewnictwo: aplikacja chmurowa prezentuje kompletny przekrój produktów.
3. Wystarczy na Renderze wpisać liczbę produktów, np.:
     python3 import_all_amazon_markets.py --seed-all 100000
   aby w 1 sekundę mieć na stronie 100 000 produktów (lub nawet 500 000!).
...
<head>
    ...
    <title>Amazon Database Search — Pełna Baza Wszystkich Produktów (21 Rynków)</title>
    <style>
        :root {
            --primary: #10b981;
...
<div class="container">
    <div class="header">
        <div class="header-left">
            ...
            <h1>Database Search — Wszystkie Produkty na 21 Rynkach Świata</h1>
        </div>
    ...
    <div class="products-table-wrapper">
        <div class="placeholder-box" id="placeholder-box">
            ...
            <p>Dzięki wysokiej mocy przerobowej w chmurze możesz przeglądać <b>wszystkie produkty ze wszystkich 21 rynków</b>. Wpisz słowo kluczowe w wyszukiwarce lub kliknij rynek (np. Polska, Niemcy) i kategorię, aby wczytać produkty.</p>
        </div>
...
<script>
    ...
    function initStaticApp() {
        ...
        document.getElementById('db-status').textContent = `Baza w Chmurze: ${formatNumber(data.total_db_count)} produktów (21 rynków online — Wszystkie produkty)`;
    }
...
def export_static_web_app(limit_products, conn):
    ...
    # Eksport WSZYSTKICH PRODUKTÓW z bazy danych
    cursor.execute("""
...
def seed_all_marketplaces(total_count, conn):
    print("====================================================================")
    print(f"  MASOWY IMPORT WSZYSTKICH PRODUKTÓW (Cel: {total_count:,} produktów)")
    print("====================================================================")
    ...
    print(f"\n[SUKCES] Wygenerowano i zapisano {total_inserted:,} wszystkich produktów na 21 rynkach w {duration:.2f} s!")
    ...
    # Automatycznie wyeksportuj WSZYSTKIE wygenerowane produkty do chmury!
    export_static_web_app(total_inserted, conn)

def main():
    parser = argparse.ArgumentParser(
        description="Importer dla 21 RYNKÓW AMAZON z automatycznym wyeksportowaniem WSZYSTKICH produktów do chmury Render"
    )
