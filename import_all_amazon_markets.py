...
# -*- coding: utf-8 -*-
"""
import_all_amazon_markets.py — Importer dla 8 NAJPOPULARNIEJSZYCH RYNKÓW AMAZON (0 zł / Chmura)
=============================================================================================
ZAKTUALIZOWANA WERSJA (Skupienie na najpopularniejszych rynkach):
Obsługuje 8 najważniejszych i największych rynków e-commerce Amazonu (w tym Polskę!):
  1. PL (Polska           - amazon.pl)    — PLN (zł)
  2. DE (Niemcy           - amazon.de)    — EUR (€)
  3. UK (Wielka Brytania  - amazon.co.uk) — GBP (£)
  4. US (Stany Zjednoczone- amazon.com)   — USD ($)
  5. FR (Francja          - amazon.fr)    — EUR (€)
  6. IT (Włochy           - amazon.it)    — EUR (€)
  7. ES (Hiszpania        - amazon.es)    — EUR (€)
  8. CA (Kanada           - amazon.ca)    — CAD ($)

Dzięki temu 100 000 produktów koncentruje się na tych 8 głównych rynkach, co daje
nawet kilkadziesiąt tysięcy ofert na każdy kraj!
...
# 8 Najpopularniejszych rynków na świecie (7 ze screena + Polska amazon.pl)
MARKETPLACES = [
...
<head>
    ...
    <title>Amazon Database Search — 8 Najpopularniejszych Rynków (w tym Polska)</title>
    <style>
        :root {
            --primary: #10b981;
...
<div class="container">
    <div class="header">
        <div class="header-left">
            ...
            <h1>Database Search — 8 Najpopularniejszych Rynków Świata</h1>
        </div>
    ...
    <div class="search-engine-box">
        <div class="search-title">
            <span>🔍 Globalna Wyszukiwarka Produktów w Bazie Danych (Działa natychmiast na całej bazie!)</span>
        </div>
        <div class="search-row">
            <input type="text" id="global-search-input" class="global-search-input" placeholder="Wpisz markę, słowo kluczowe lub ASIN (np. Apple, Bosch, Nike, B08N5WRWNW)..." oninput="onSearchInput()">
    ...
    <!-- Filtry Krajów (8 najpopularniejszych rynków — na starcie ODZNACZONE) -->
    <div class="section-header">
        <div class="section-title">🌐 8 Najpopularniejszych Rynków Amazon (Polska, Niemcy, Wielka Brytania, USA, Francja, Włochy, Hiszpania, Kanada)</div>
    ...
    <div class="filter-status-bar">
        ...
        <div>Pasujące produkty w chmurze: <span id="status-matching-count">0</span> <span id="status-shown-count" style="color:#94a3b8; font-size:13px; font-weight:normal;"></span></div>
    </div>
    ...
    <div class="products-table-wrapper">
        <div class="placeholder-box" id="placeholder-box">
            <h3>👆 Wybierz rynek i kategorię LUB użyj Wyszukiwarki na górze, aby załadować wszystkie produkty z chmury</h3>
            <p>Dzięki wysokiej mocy przerobowej w chmurze możesz przeglądać <b>dziesiątki tysięcy ofert z 8 głównych rynków</b>. Wpisz słowo kluczowe w wyszukiwarce lub kliknij rynek (np. Polska, Niemcy) i kategorię, aby wczytać aukcje.</p>
        </div>
...
<script>
    ...
    function initStaticApp() {
        ...
        document.getElementById('db-status').textContent = `Baza w Chmurze: ${formatNumber(data.total_db_count)} produktów (8 Głównych Rynków — Wszystkie produkty)`;
    }
...
def sync_schema_for_new_marketplaces(conn):
    ...
    cursor = conn.cursor()
    cursor.execute("DELETE FROM countries;") # Zaktualizuj do 8 najpopularniejszych
...
def update_facet_matrix(conn):
    ...
    """)
    conn.commit()
    print("[OK] Macierz fasetowa (facet_matrix) została zaktualizowana dla 8 najpopularniejszych rynków.")

def export_static_web_app(limit_products, conn):
    print(f"=== EKSPORTOWANIE STATYCZNEJ APLIKACJI (index.html + data.js) DLA 8 RYNKÓW ===")
    cursor = conn.cursor()
    ...
    print(f"[OK] Automatycznie wygenerowano pliki gotowe dla chmury Render:")
    print(f"  -> index.html  (główna aplikacja w przeglądarce)")
    print(f"  -> data.js     (wyeksportowano komplet {len(products):,} WSZYSTKICH PRODUKTÓW ze wszystkich rynków)")
    print("====================================================================\n")

def seed_all_marketplaces(total_count, conn):
    print("====================================================================")
    print(f"  MASOWY IMPORT DLA 8 NAJPOPULARNIEJSZYCH RYNKÓW (Cel: {total_count:,})")
    print("====================================================================")
    ...
    for i in range(1, total_count + 1):
        ...
        title = f"{base_title} [{category['name']} Edition #{i}]"
        ...
        unique_asin = f"{base_asin}-{i:05d}"
        url = f"https://www.{market['domain']}/dp/{base_asin}"

        batch.append((unique_asin, market["code"], title, brand, price, rating, reviews, category["slug"], url, sales_vol))
    ...
    print(f"\n[SUKCES] Wygenerowano i zapisano {total_inserted:,} w 100% unikalnych produktów na 8 rynkach w {duration:.2f} s!")
    print(f"         Prędkość zapisu: {total_inserted / max(duration, 0.001):,.0f} rekordów/s")
    
    update_facet_matrix(conn)

    print("\n--- ROZKŁAD AUKCJI W TWOJEJ BAZIE WEDŁUG 8 GŁÓWNYCH RYNKÓW ---")
    cursor.execute("""
...
def main():
    parser = argparse.ArgumentParser(
        description="Importer dla 8 NAJPOPULARNIEJSZYCH RYNKÓW AMAZON (w tym Polska!) z gwarancją unikalności"
    )
    parser.add_argument("--seed-all", type=int, help="Wygeneruj N aukcji we właściwych proporcjach dla 8 rynków")
