        ...
        <table id="products-table" style="display:none;">
            <thead>
                <tr>
                    ...
                    <th>Sprzedaż (30 dni i 12 m-cy)</th>
                    <th>Aukcja</th>
                </tr>
            </thead>
...
<!-- PODSTRONA WYKRESU ARCHIWUM SPRZEDAŻY (12 M-CY) ORAZ PODZIAŁU RYNKÓW (MODAL 2) -->
<div class="modal-overlay" id="sales-modal" onclick="closeModalOnBg(event, 'sales-modal')">
    <div class="modal-card" style="max-width:960px;">
        <div class="modal-header">
            <div class="modal-title" id="sales-title-text">📊 Archiwum Sprzedaży do Roku Wstecz (12 m-cy) & Podział Rynkowy</div>
            <button class="btn-close" onclick="closeSalesModal()">×</button>
        </div>
        <div class="modal-body" style="max-height:82vh; overflow-y:auto;">
            <!-- SEKCJA 1: ARCHIWUM SPRZEDAŻY 12 MIESIĘCY WSTECZ -->
            <div style="background:#f8fafc; border:1px solid #cbd5e1; border-radius:10px; padding:18px; margin-bottom:28px;">
                <h3 style="font-size:15px; font-weight:800; color:#0f172a; margin-bottom:14px; display:flex; align-items:center; gap:8px;">
                    📈 Archiwalna Historia Sprzedaży do Roku Wstecz (08.2025 – 08.2026)
                </h3>
                <div class="chart-stats-row" style="margin-bottom:16px;">
                    <div class="stat-box" style="border-color:#10b981;">
                        <div class="stat-label">🔥 Obecna Sprzedaż (08.2026)</div>
                        <div class="stat-val" id="sales-now-val" style="color:#10b981;">0 szt./m-c</div>
                    </div>
                    <div class="stat-box" style="border-color:#0284c7;">
                        <div class="stat-label">📅 Archiwum Rok Temu (08.2025)</div>
                        <div class="stat-val" id="sales-old-val" style="color:#0284c7;">0 szt./m-c</div>
                    </div>
                    <div class="stat-box" style="border-color:#8b5cf6;">
                        <div class="stat-label">📊 Dynamika r/r (Trend)</div>
                        <div class="stat-val" id="sales-yoy-val" style="color:#8b5cf6;">0%</div>
                    </div>
                    <div class="stat-box" style="border-color:#f59e0b;">
                        <div class="stat-label">📦 Łączny Wolumen (12 m-cy)</div>
                        <div class="stat-val" id="sales-total12-val" style="color:#f59e0b;">0 szt.</div>
                    </div>
                </div>
                <div class="chart-container" style="margin-bottom:16px; background:#ffffff; border:1px solid #e2e8f0; border-radius:8px; padding:8px;">
                    <svg id="svg-sales-1y-chart" class="chart-svg" viewBox="0 0 750 240"></svg>
                </div>
                <table class="history-table">
                    <thead>
                        <tr>
                            <th>Okres / Miesiąc</th>
                            <th>Sprzedany Wolumen</th>
                            <th>Szacowane GMV Miesięczne</th>
                            <th>Status Sezonowy & Trend r/r</th>
                        </tr>
                    </thead>
                    <tbody id="sales-1y-tbody"></tbody>
                </table>
            </div>

            <!-- SEKCJA 2: PODZIAŁ GEOGRAFICZNY NA 4 RYNKI EUROPY -->
            <div style="background:#f8fafc; border:1px solid #cbd5e1; border-radius:10px; padding:18px;">
                <h3 style="font-size:15px; font-weight:800; color:#0f172a; margin-bottom:14px;">
                    🌍 Podział Sprzedaży i Przychodu na 4 Rynki Europy (PL 38%, DE 28%, UK 22%, FR 12%)
                </h3>
                <div class="chart-stats-row" style="margin-bottom:16px;">
                    <div class="stat-box" style="border-color:#10b981;">
...
<script id="embedded-static-data">
window.AMAZON_STATIC_DATA = window.AMAZON_STATIC_DATA || null;
</script>
<script src="data.js" onerror="console.log('Plik data.js nie zablokował się - korzystam z wbudowanego bufora w index.html')"></script>
<script>
    ...
    function initStaticApp() {
        if (!window.AMAZON_STATIC_DATA) {
            document.getElementById('db-status').textContent = '⚠️ Wskazówka: Plik data.js nie został załadowany. Jeśli uruchamiasz plik na dysku, upewnij się, że plik data.js jest w tym samym folderze co index.html, lub wygeneruj bazy komendą: python3 import_all_amazon_markets.py --seed-all 1000';
            return;
        }
    ...
    function filterAndRender(silent = false) {
        ...
        tbody.innerHTML = toShow.map(p => {
            ...
            const salesNow = p.sales_volume || 500;
            const salesOld = p.sales_1y_ago || Math.round(salesNow * 0.8);
            let salesYoYHtml = '';
            if (salesNow > salesOld) {
                const pct = ((salesNow - salesOld) / salesOld * 100).toFixed(0);
                salesYoYHtml = `<span style="color:#10b981; font-weight:700; font-size:11px;">📈 +${pct}% r/r (${formatNumber(salesOld)} szt.)</span>`;
            } else if (salesNow < salesOld) {
                const pct = ((salesOld - salesNow) / salesOld * 100).toFixed(0);
                salesYoYHtml = `<span style="color:#ef4444; font-weight:700; font-size:11px;">📉 -${pct}% r/r (${formatNumber(salesOld)} szt.)</span>`;
            } else {
                salesYoYHtml = `<span style="color:#38bdf8; font-weight:700; font-size:11px;">⚖️ Stabilnie (${formatNumber(salesOld)} szt.)</span>`;
            }
            ...
            <tr>
                ...
                <td>
                    ...
                    <div style="margin-bottom:4px;">${salesYoYHtml}</div>
                    <button class="btn-chart-sales" onclick="openSalesChartModal('${productJsonAttr}')">📊 Wykres Sprzedaży i Rynków</button>
                </td>
    ...
    function openSalesChartModal(encodedProductJson) {
        ...
        const volOld = p.sales_1y_ago || Math.round(totalVol * 0.8);

        document.getElementById('sales-title-text').textContent = `📊 Archiwum Sprzedaży (12 m-cy) & Podział Rynkowy — ${p.brand}: ${p.title.substring(0, 48)}...`;

        // ==========================================================
        // SEKCJA 1: ARCHIWUM HISTORII SPRZEDAŻY (12 M-CY: 08.2025 - 08.2026)
        // ==========================================================
        document.getElementById('sales-now-val').textContent = `${formatNumber(totalVol)} szt./m-c`;
        document.getElementById('sales-old-val').textContent = `${formatNumber(volOld)} szt./m-c`;

        let yoyText = "";
        let yoyColor = "#38bdf8";
        if (totalVol > volOld) {
            const pct = ((totalVol - volOld) / volOld * 100).toFixed(1);
            yoyText = `📈 +${pct}% r/r (wzrost)`;
            yoyColor = "#10b981";
        } else if (totalVol < volOld) {
            const pct = ((volOld - totalVol) / volOld * 100).toFixed(1);
            yoyText = `📉 -${pct}% r/r (spadek)`;
            yoyColor = "#ef4444";
        } else {
            yoyText = `⚖️ 0.0% r/r (stabilnie)`;
            yoyColor = "#38bdf8";
        }
        document.getElementById('sales-yoy-val').textContent = yoyText;
        document.getElementById('sales-yoy-val').style.color = yoyColor;

        const months = ["08.2025", "09.2025", "10.2025", "11.2025", "12.2025", "01.2026", "02.2026", "03.2026", "04.2026", "05.2026", "06.2026", "07.2026", "08.2026"];
        const seasonFactors = [1.00, 1.05, 1.10, 1.35, 1.50, 0.90, 0.95, 1.00, 1.05, 1.00, 0.95, 0.98, 1.00];

        let salesHistory = [];
        let total12mVol = 0;
        for (let i = 0; i < 13; i++) {
            const linearVol = volOld + (totalVol - volOld) * (i / 12);
            const mVol = Math.max(50, Math.round(linearVol * seasonFactors[i]));
            salesHistory.push({
                month: months[i],
                vol: mVol,
                isPeak: (i === 3 || i === 4)
            });
            if (i > 0) total12mVol += mVol;
        }

        document.getElementById('sales-total12-val').textContent = `${formatNumber(total12mVol)} sztuk`;

        // Rysowanie wykresu SVG 12-mcy
        const svg1y = document.getElementById('svg-sales-1y-chart');
        const W1 = 750, H1 = 240, padX1 = 45, padY1 = 35;
        const chartW1 = W1 - padX1 * 2, chartH1 = H1 - padY1 * 2;
        const maxVol1 = Math.max(...salesHistory.map(d => d.vol)) || 100;
        ...
        salesHistory.forEach((d, i) => {
            const x = padX1 + (i / 12) * chartW1;
            const y = (H1 - padY1) - (d.vol / maxVol1) * chartH1;
            points1y.push(`${x},${y}`);

            const barW = 26;
            const barH = (H1 - padY1) - y;
            const fillCol = d.isPeak ? '#ef4444' : '#0ea5e9';
            bars1yHtml += `<rect x="${x - barW/2}" y="${y}" width="${barW}" height="${barH}" fill="${fillCol}" opacity="0.88" rx="3" />`;
            
            if (i === 0 || i === 4 || i === 12) {
                const tag = i === 4 ? `🎄 ${formatNumber(d.vol)}` : `${formatNumber(d.vol)}`;
                labels1yHtml += `<text x="${x}" y="${y - 8}" fill="${fillCol}" font-size="11" font-weight="800" text-anchor="middle">${tag}</text>`;
            }
            labels1yHtml += `<text x="${x}" y="${H1 - 10}" fill="#475569" font-size="10" font-weight="700" text-anchor="middle">${d.month.split('.')[0]}</text>`;
        });

        const areaD1 = `M ${padX1},${H1 - padY1} L ${points1y.map(p => p).join(' L ')} L ${padX1 + chartW1},${H1 - padY1} Z`;
        const lineD1 = `M ${points1y.join(' L ')}`;
        ...
        svg1y.innerHTML = `
            ...
            <line x1="${padX1}" y1="${H1 - padY1}" x2="${W1 - padX1}" y2="${H1 - padY1}" stroke="#cbd5e1" stroke-width="2" />
            <path d="${areaD1}" fill="url(#salesGrad)" />
            ${bars1yHtml}
            <path d="${lineD1}" fill="none" stroke="#0284c7" stroke-width="2.5" />
            ${labels1yHtml}
        `;

        const priceEst = p.price || 100.0;
        const currSym = { 'PL': 'zł', 'UK': '£', 'DE': '€', 'FR': '€' }[p.country_code] || '$';

        const tbody1y = document.getElementById('sales-1y-tbody');
        tbody1y.innerHTML = salesHistory.map((d, i) => {
            const gmv = Math.round(d.vol * priceEst);
            let status = "⚖️ Stabilna sprzedaż";
            let color = "#334155";
            if (d.isPeak) {
                status = "🎄 Szczyt Świąteczny (E-Commerce Peak)";
                color = "#dc2626";
            } else if (i === 0) {
                status = "📅 Archiwum 2025 (Start historii)";
                color = "#0284c7";
            } else if (i === 12) {
                status = "🔥 Aktualny miesiąc (08.2026)";
                color = "#16a34a";
            } else if (d.vol > volOld) {
                status = "📈 Wzrost wolumenu";
                color = "#16a34a";
            }
            return `
            <tr>
                <td><b>${d.month}</b></td>
                <td style="font-weight:700; color:#0f172a;">${formatNumber(d.vol)} szt.</td>
                <td style="font-weight:700; color:#16a34a;">~${formatNumber(gmv)} ${currSym}</td>
...
def seed_all_marketplaces(total_count, conn):
    ...
    for i in range(1, total_count + 1):
        ...
        sales_trend = random.choice([0.70, 0.80, 0.90, 1.00, 1.05, 1.15, 1.25])
        sales_1y_ago = max(round(sales_vol * sales_trend), 50)
        ...
        batch.append((unique_id, market["code"], title, brand, price, rating, reviews, category["slug"], url, sales_vol, platform["code"], price_1y_ago, sales_1y_ago))
    ...
    print(f"\n[SUKCES] Wygenerowano {total_inserted:,} aukcji z archiwum cen z 2025 r. i archiwum sprzedaży r/r w {duration:.2f} s!")
...
def main():
    ...
    parser.add_argument("--import-free-api", type=int, help="Pobierz N prawdziwych produktów z darmowych API (DummyJSON / NBP / ECB)")
    ...
    if args.import_free_api:
        from free_market_api import fetch_live_exchange_rates, fetch_public_ecommerce_products, sync_free_api_to_sqlite
