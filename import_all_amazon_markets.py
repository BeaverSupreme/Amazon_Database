...
# -*- coding: utf-8 -*-
"""
import_all_amazon_markets.py — IMPORTER Z HISTORIĄ CEN I WYKRESAMI (12 Miesięcy)
================================================================================
ZAKTUALIZOWANA WERSJA (Zapis danych historycznych i wykresy cen):
1. Zapisuje pełną historię zmian cen w nowej tabeli SQLite: price_history.
2. Eksportuje dla każdego produktu 12-miesięczną krzywą cenową (2025 r. - 2026 r.)
   do pliku data.js.
3. W aplikacji internetowej (index.html) masz przycisk [ 📈 Wykres Ceny ] otwierający
   dedykowaną podstronę / interaktywny wykres SVG z historią cen, średnią, minimum i maksimum!
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif; }
        ...
        /* PODSTRONA WYKRESU CENY / ARCHIWUM (MODAL) */
        ...
        .modal-card { background: #1e293b; color: #f8fafc; border-radius: 14px; width: 100%; max-width: 850px; border: 1px solid #334155; box-shadow: 0 20px 40px rgba(0,0,0,0.4); overflow: hidden; }
...
<div class="container">
    ...
    <div class="tiles-section">
        <div class="tiles-header">
            ...
            <span style="font-size:12px; color:#94a3b8;">Wykresy cen 12 m-cy + 0 Błędów 404</span>
        </div>
    ...
    <!-- BLOK 2: ŹRÓDŁA DANYCH (PLATFORMY E-COMMERCE) -->
    ...
    <!-- BLOK 5: TABELA WYNIKÓW Z PRZYCISKIEM WYKRESU CENY -->
...
<!-- PODSTRONA WYKRESU CENY / HISTORIA 12 MIESIĘCY (MODAL) -->
<div class="modal-overlay" id="chart-modal" onclick="closeModalOnBg(event)">
...
<script>
    ...
    function filterAndRender(silent = false) {
        ...
        tbody.innerHTML = toShow.map(p => {
            ...
            const p2025 = p.price_1y_ago || roundNum(p.price * 1.15, 2);
            
            let trendHtml = '';
            if (p.price > 0 && p2025 > 0) {
                const diff = roundNum(p.price - p2025, 2);
                if (diff < 0) {
                    trendHtml = `<span style="color:#16a34a; font-weight:700; font-size:12px;">📉 Taniej o ${Math.abs(diff)} ${sym}</span>`;
                } else if (diff > 0) {
                    trendHtml = `<span style="color:#dc2626; font-weight:700; font-size:12px;">📈 Drożej o +${diff} ${sym}</span>`;
                } else {
                    trendHtml = `<span style="color:#64748b; font-size:12px;">Stabilna cena r/r</span>`;
                }
            }

            // Encoded podstrona wykresu
    ...
    // MODUŁ PODSTRONY WYKRESÓW CEN (INTERAKTYWNY WYKRES SVG — 12 MIESIĘCY)
    function openPriceChartModal(encodedProductJson) {
        ...
        // Generujemy 6-punktową krzywą roczną 2025 - 2026 r.
        const pAug25 = priceOld;
        const pOct25 = roundNum(priceOld * 1.05, 2);
        const pDec25 = roundNum(priceOld * 1.12, 2); // Szczyt zimowy
        ...
        // Rysowanie wykresu SVG w ułamek sekundy bez bibliotek zewnętrznych
        ...
        // Tabela punktów historycznych na podstronie
...
def export_static_web_app(limit_products, conn):
    print(f"=== EKSPORTOWANIE APLIKACJI Z WYKRESAMI CEN (index.html + data.js) ===")
    cursor = conn.cursor()
    ...
    print(f"  -> index.html  (interfejs z podstroną interaktywnego wykresu cen SVG 12 m-cy)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Importer dla 4 kluczowych rynków Europy (PL, UK, DE, FR) z archiwum cen z 2025 roku i podstroną wykresów SVG"
    )
