    ...
    try:
        ...
        for asin, country_code, title, brand, category_slug, url, sales_vol, price, p_1y, platform_code, s_1y in all_rows:
            ...
            if not url or "/oferta/" in url or "/itm/" in url or "/item/" in url or ".amazon.it" in url or ".amazon.es" in url or ".amazon.ca" in url or ".amazon.com" in url or "B0PBBWSQAG" in url:
                needs_update = True
            if not sales_vol or sales_vol == 0 or not p_1y or p_1y == 0 or not s_1y or s_1y == 0:
                needs_update = True
...
<script src="data.js" onerror="console.log('Plik data.js nie załadował się - ładuję dane przez API lub bufor')"></script>
<script>
    ...
    function initStaticApp() {
        if (!window.AMAZON_STATIC_DATA) {
            fetch('/api/facets').then(r => r.json()).then(facets => {
                fetch('/api/products?limit=1000').then(r => r.json()).then(prods => {
                    window.AMAZON_STATIC_DATA = {
                        total_db_count: facets.total_db_count,
                ...
                document.getElementById('db-status').textContent = '⚠️ Wskazówka: Plik data.js nie został załadowany. Upewnij się, że plik data.js jest w tym samym folderze co index.html, lub wygeneruj komendą: python3 import_all_amazon_markets.py --seed-all 1000';
            });
            return;
        }
